...
# Grab DFS salaries
dfs_data = None
if not os.path.isfile(f'data/{SEASON}/week{week}/dfs_salaries.json'):
...
if not s3_client.upload_file(season=SEASON, week=week, filename='week_matchups.binarypb'):
...
if not s3_client.upload_file(season=SEASON, week=week, filename='player_pool.binarypb'):
